...
async function fetchEndpoint(endpoint: string): Promise<{ data: RegistryItem[]; error: string | null }> {
  if (inflight.has(endpoint)) {
    const data = await inflight.get(endpoint)!
  ...
  inflight.set(endpoint, promise.catch(() => { inflight.delete(endpoint); return [] }).then(r => { inflight.delete(endpoint); return r }))
  try {
...
export function useRegistry<T extends RegistryItem>(endpoint: string) {
  ...
  const load = useCallback(async () => {
    ...
    setFetchError(null)
    const { data, error } = await fetchEndpoint(endpoint)
    if (error) {
      setFetchError(error)
    } else {
      setItems(data as T[])
  ...
  return { items, loading, saving, fetchError, create, update, remove, reload: load }
}
