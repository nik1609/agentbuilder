/**
 * POST /api/runs/:runId/hitl/approve
 * Body: { feedback?: string }
 * Approves the HITL checkpoint and resumes execution.
 * Delegates to the /resume handler.
 */
...
export async function POST(
  ...
  const feedback: string | undefined = (body.feedback as string)?.trim() || undefined
  ...
  // Append approval event to trace
  const trace = (run.trace ?? []) as unknown[]
  trace.push({ type: 'hitl_message', ts: Date.now(), message: feedback ?? '(approved)', data: { role: 'human', content: feedback ?? '(approved)', action: 'approve' } })

  const result = await executeAgent(
    schema as never, run.input as Record<string, unknown>,
    run.agent_id, runId, undefined,
    { checkpointNodeId, partialOutput: savedOutput?.partial, feedback },
