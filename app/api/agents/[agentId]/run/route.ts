...
export async function POST(
  ...
  // ── 1. Auth — session first (dashboard), then API key (external) ──────────
  ...
  // API Key tracking (for call counts)
  ...
  if (!resolvedUserId) {
    // Allow test mode without auth for local development
    if (!isTestMode) {
      return NextResponse.json(
        { error: 'Unauthorized. Pass X-AgentHub-Key header or sign in.' },
        { status: 401 }
      )
    }
  }
  ...
  if (!isTestMode && apiKeyHeader) {
    ...
    const { data } = await db
      .from('api_keys')
      .select('id, key_prefix, total_calls')
      .eq('key_hash', keyHash)
      .eq('is_active', true)
      .single()
    ...
    if (!data) {
      return NextResponse.json(
        { error: 'Invalid or revoked API key. Pass X-AgentHub-Key header.' },
        { status: 403 }
      )
    }
    keyRecord = data
  ...
  // ── 3. Load model configs for this user ──────────────────────────────────
  ...
  // ── 3b. Load tools and inject into schema nodes ──────────────────────────
  ...
  let result
  try {
    result = await executeAgent(schema as never, input, agentId, runId, undefined, undefined, modelConfigs)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    await db.from('agent_runs').update({
      status: 'failed', error: msg, latency_ms: Date.now() - startTime, trace: [],
    }).eq('id', runId)
    return NextResponse.json(
      { runId, agentId, agentName: agent.name, status: 'failed', error: msg, output: null, tokens: 0, latencyMs: Date.now() - startTime },
      { status: 500 }
    )
  }
  ...
  if (keyRecord) {
    await db.from('api_keys').update({
      total_calls: (keyRecord.total_calls ?? 0) + 1,
    ...
    async start(controller) {
      ...
      try {
        const result = await executeAgent(
          agent.schema, { message }, agentId, runId,
